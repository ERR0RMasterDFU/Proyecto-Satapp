...
public interface CategoriaRepository extends JpaRepository<Categoria, Long> {
    ...
    @Query("""
        select new com.salesianostriana.dam.proyecto_satapp.dto.categoria.GetCategoriaSinListasDto(
            c.nombre, COALESCE(c.categoriaPadre.nombre, 'Ninguna')
        )
    ...
    @Query("""
        select new com.salesianostriana.dam.proyecto_satapp.dto.categoria.GetSubCategoriaDto(
            c.nombre
        )
        from Categoria c
        where c.categoriaPadre.id = ?1
    """)
    List<GetSubCategoriaDto> findSubCategoriasDtoById(Long id);
